...
async function exportBackup() {
  ...
  const payload = {
    ...
    customFields,
    welcome: {...welcomeBackupSnapshot(), canonicalModel: buildCanonicalWelcomeModel(), affiliateLogoAsset: await getAffiliateAsset('affiliate-primary-logo'), auditPhotoAssets: (await Promise.all(getQrTouchpoints().filter(x=>x.photoKey).map(async x=>await getAuditAsset(x.photoKey)))).filter(Boolean)},
    records: serialized
...
async function restoreWelcomeBackup(welcome) {
  ...
  if(welcome.affiliateLogoAsset?.dataUrl){
    await saveAffiliateAsset('affiliate-primary-logo',welcome.affiliateLogoAsset.dataUrl,welcome.affiliateLogoAsset);
    restored=true;
  }
  if(Array.isArray(welcome.auditPhotoAssets)){
    for(const asset of welcome.auditPhotoAssets){if(asset?.key&&asset?.dataUrl){await saveAuditAsset(asset.key,asset.dataUrl,asset);restored=true}}
  }
  if(welcome.language){
...
async function importBackupFile(file) {
  ...
  const welcomeRestored = payload.version >= 2 ? await restoreWelcomeBackup(payload.welcome) : false;
...
if ('serviceWorker' in navigator) {
  ...
  navigator.serviceWorker.register('./service-worker.js?v=0.44.2', { updateViaCache: 'none' })
    .then(async reg => {
...
const PAPA_GOLF_AUDIT_ASSET_DB='papa-golf-audit-assets-v01';
const PAPA_GOLF_AUDIT_ASSET_STORE='touchpointPhotos';
let pendingQrTouchpointPhoto=null;
...
function openAuditAssetDb(){return new Promise((resolve,reject)=>{const req=indexedDB.open(PAPA_GOLF_AUDIT_ASSET_DB,1);req.onupgradeneeded=()=>{if(!req.result.objectStoreNames.contains(PAPA_GOLF_AUDIT_ASSET_STORE))req.result.createObjectStore(PAPA_GOLF_AUDIT_ASSET_STORE,{keyPath:'key'})};req.onsuccess=()=>resolve(req.result);req.onerror=()=>reject(req.error)})}
async function saveAuditAsset(key,dataUrl,meta={}){const db=await openAuditAssetDb();return new Promise((resolve,reject)=>{const tx=db.transaction(PAPA_GOLF_AUDIT_ASSET_STORE,'readwrite');tx.objectStore(PAPA_GOLF_AUDIT_ASSET_STORE).put({key,dataUrl,...meta,updatedAt:new Date().toISOString()});tx.oncomplete=()=>{db.close();resolve(true)};tx.onerror=()=>{db.close();reject(tx.error)}})}
async function getAuditAsset(key){try{const db=await openAuditAssetDb();return await new Promise((resolve,reject)=>{const tx=db.transaction(PAPA_GOLF_AUDIT_ASSET_STORE,'readonly'),req=tx.objectStore(PAPA_GOLF_AUDIT_ASSET_STORE).get(key);req.onsuccess=()=>{db.close();resolve(req.result||null)};req.onerror=()=>{db.close();reject(req.error)}})}catch{return null}}
async function deleteAuditAsset(key){try{const db=await openAuditAssetDb();return await new Promise((resolve,reject)=>{const tx=db.transaction(PAPA_GOLF_AUDIT_ASSET_STORE,'readwrite');tx.objectStore(PAPA_GOLF_AUDIT_ASSET_STORE).delete(key);tx.oncomplete=()=>{db.close();resolve(true)};tx.onerror=()=>{db.close();reject(tx.error)}})}catch{return false}}
function prepareAuditPhoto(file){return new Promise((resolve,reject)=>{if(!file||!file.type?.startsWith('image/'))return reject(new Error('Choose an image file.'));const reader=new FileReader();reader.onerror=()=>reject(reader.error||new Error('Could not read photo.'));reader.onload=()=>{const img=new Image();img.onerror=()=>reject(new Error('Could not decode photo.'));img.onload=()=>{const max=1280,scale=Math.min(1,max/Math.max(img.naturalWidth||1,img.naturalHeight||1));const w=Math.max(1,Math.round(img.naturalWidth*scale)),h=Math.max(1,Math.round(img.naturalHeight*scale));const c=document.createElement('canvas');c.width=w;c.height=h;const ctx=c.getContext('2d');ctx.drawImage(img,0,0,w,h);let dataUrl='';try{dataUrl=c.toDataURL('image/webp',.82)}catch{}if(!dataUrl||dataUrl==='data:,')dataUrl=c.toDataURL('image/jpeg',.82);resolve({dataUrl,name:file.name||'audit-photo.jpg',width:w,height:h,originalBytes:file.size||0})};img.src=String(reader.result||'')};reader.readAsDataURL(file)})}
function renderPendingQrTouchpointPhoto(){const host=document.getElementById('qrTouchpointPhotoPreview'),clear=document.getElementById('clearQrTouchpointPhotoBtn');if(!host)return;host.classList.toggle('hidden',!pendingQrTouchpointPhoto);clear?.classList.toggle('hidden',!pendingQrTouchpointPhoto);host.innerHTML=pendingQrTouchpointPhoto?`<img src="${escapeHtml(pendingQrTouchpointPhoto.dataUrl)}" alt="Audit photo preview"><span>${escapeHtml(pendingQrTouchpointPhoto.name||'Audit photo ready')}</span>`:''}
async function selectQrTouchpointPhoto(file){if(!file)return;const host=document.getElementById('qrTouchpointPhotoPreview');if(host){host.classList.remove('hidden');host.textContent='Preparing photo…'}try{pendingQrTouchpointPhoto=await prepareAuditPhoto(file);renderPendingQrTouchpointPhoto()}catch(error){pendingQrTouchpointPhoto=null;if(host)host.textContent=`Photo could not be prepared: ${error.message||error}`}}
function clearPendingQrTouchpointPhoto(){pendingQrTouchpointPhoto=null;['qrTouchpointCameraInput','qrTouchpointPhotoInput'].forEach(id=>{const e=document.getElementById(id);if(e)e.value=''});renderPendingQrTouchpointPhoto()}
...
function resetQrTouchpointForm(){editingQrTouchpointId='';['qrTouchpointLocation','qrTouchpointExisting','qrTouchpointLabel','qrTouchpointNote'].forEach(id=>{const el=document.getElementById(id);if(el)el.value=''});const st=document.getElementById('qrTouchpointStatus');if(st)st.value='proposed';const pr=document.getElementById('qrTouchpointPriority');if(pr)pr.value='medium';const btn=document.getElementById('addQrTouchpointBtn');if(btn)btn.textContent='+ Add Touchpoint';document.getElementById('cancelQrTouchpointEditBtn')?.classList.add('hidden');clearPendingQrTouchpointPhoto()}
async function editQrTouchpoint(id){const item=getQrTouchpoints().find(x=>x.id===id);if(!item)return;editingQrTouchpointId=id;const vals={qrTouchpointLocation:item.location,qrTouchpointExisting:item.existing,qrTouchpointDestination:item.destination||'home',qrTouchpointLabel:item.label,qrTouchpointPriority:item.priority||'medium',qrTouchpointStatus:item.status||'proposed',qrTouchpointNote:item.note};for(const [id2,v] of Object.entries(vals)){const el=document.getElementById(id2);if(el)el.value=v||''}pendingQrTouchpointPhoto=null;if(item.photoKey){const a=await getAuditAsset(item.photoKey);if(a?.dataUrl)pendingQrTouchpointPhoto={dataUrl:a.dataUrl,name:a.name||'Audit photo',width:a.width,height:a.height,existingKey:item.photoKey}}renderPendingQrTouchpointPhoto();const btn=document.getElementById('addQrTouchpointBtn');if(btn)btn.textContent='Save changes';document.getElementById('cancelQrTouchpointEditBtn')?.classList.remove('hidden');document.querySelector('.qr-touchpoint-form')?.scrollIntoView({behavior:'smooth',block:'start'})}
async function hydrateQrTouchpointPhotos(){const cards=[...document.querySelectorAll('#qrTouchpointList [data-touchpoint-photo-key]')];await Promise.all(cards.map(async el=>{const key=el.dataset.touchpointPhotoKey;if(!key)return;const asset=await getAuditAsset(key);if(asset?.dataUrl){el.src=asset.dataUrl;el.classList.remove('hidden')}}))}
function renderQrTouchpoints(){
  const host=document.getElementById('qrTouchpointList'),summary=document.getElementById('qrTouchpointSummary');if(!host)return;
  const items=getQrTouchpoints();if(summary){const high=items.filter(x=>x.priority==='high').length;summary.textContent=items.length?`${items.length} touchpoint${items.length===1?'':'s'} recorded${high?` · ${high} high priority`:''}.`:'No touchpoints recorded yet.'}
  if(!items.length){host.innerHTML='<div class="qr-touchpoint-empty">Walk the property with the manager and add opportunities as you see them.</div>';return}
  const shared=sharedBackendCanUseStableLink();host.innerHTML=items.map(item=>{const url=welcomeSectionUrl(item.destination||'home');const qr=`https://api.qrserver.com/v1/create-qr-code/?size=220x220&margin=10&format=png&data=${encodeURIComponent(url)}`;return `<article class="qr-touchpoint-card" data-touchpoint-id="${escapeHtml(item.id)}">
      ${item.photoKey?`<img class="qr-touchpoint-photo hidden" data-touchpoint-photo-key="${escapeHtml(item.photoKey)}" alt="Audit location photo">`:''}
      <div class="qr-touchpoint-main"><div class="qr-touchpoint-flags"><div class="qr-touchpoint-priority ${escapeHtml(item.priority||'medium')}">${escapeHtml(qrTouchpointPriorityLabel(item.priority))}</div><div class="qr-touchpoint-status ${escapeHtml(item.status||'proposed')}">${escapeHtml(qrTouchpointStatusLabel(item.status))}</div><div class="qr-touchpoint-mode ${shared?'shared':'snapshot'}">${shared?'Permanent shared QR':'Local snapshot QR'}</div></div><strong>${escapeHtml(item.location||'Property touchpoint')}</strong><span>${escapeHtml(item.existing||'Existing guest information')}</span><div class="qr-touchpoint-destination">→ ${escapeHtml(QR_TOUCHPOINT_DESTINATIONS[item.destination]||'Welcome home')}</div>${item.note?`<p>${escapeHtml(item.note)}</p>`:''}<div class="qr-touchpoint-sticker">${escapeHtml(item.label||'Scan for details')}</div></div>
      <img class="qr-touchpoint-qr" src="${escapeHtml(qr)}" alt="QR preview for ${escapeHtml(item.location||'touchpoint')}"><div class="qr-touchpoint-actions"><a href="${escapeHtml(url)}" target="_blank" rel="noopener">Open destination</a><button type="button" data-copy-touchpoint="${escapeHtml(item.id)}">Copy link</button><button type="button" data-edit-touchpoint="${escapeHtml(item.id)}">Edit</button><button type="button" class="text-danger" data-remove-touchpoint="${escapeHtml(item.id)}">Remove</button></div></article>`}).join('');hydrateQrTouchpointPhotos()
}
async function addQrTouchpoint(){
  const value=id=>document.getElementById(id)?.value?.trim?.()||'',location=value('qrTouchpointLocation'),existing=value('qrTouchpointExisting');if(!location&&!existing){const el=document.getElementById('qrTouchpointLocation');if(el){el.focus();el.placeholder='Add a location or existing item first'}return}
  const items=getQrTouchpoints(),prior=editingQrTouchpointId?items.find(x=>x.id===editingQrTouchpointId):null;
  const item={id:prior?.id||papaGolfStableId('touchpoint'),gatewayId:prior?.gatewayId||sharedGateway()?.id||'',location,existing,destination:value('qrTouchpointDestination')||'home',label:value('qrTouchpointLabel')||'Scan for details',priority:value('qrTouchpointPriority')||'medium',status:value('qrTouchpointStatus')||prior?.status||'proposed',note:value('qrTouchpointNote'),createdAt:prior?.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString(),photoKey:prior?.photoKey||''};
  if(pendingQrTouchpointPhoto){item.photoKey=pendingQrTouchpointPhoto.existingKey||item.photoKey||`audit-${item.id}`;await saveAuditAsset(item.photoKey,pendingQrTouchpointPhoto.dataUrl,{name:pendingQrTouchpointPhoto.name,width:pendingQrTouchpointPhoto.width,height:pendingQrTouchpointPhoto.height})}
  if(prior){const i=items.findIndex(x=>x.id===prior.id);items[i]=item}else items.unshift(item);saveQrTouchpoints(items);resetQrTouchpointForm();renderQrTouchpoints()
}
...
async function buildQrAuditReport(){const host=document.getElementById('qrAuditReportContent');if(!host)return;const items=getQrTouchpoints(),d=effectiveWelcome(),high=items.filter(x=>x.priority==='high').length,withPhoto=items.filter(x=>x.photoKey).length;host.innerHTML=`<header class="qr-report-head"><div class="eyebrow">PAPA GOLF · PROPERTY INFORMATION UPGRADE</div><h1>${escapeHtml(d.propertyName||d.name||'Property walkthrough')}</h1><p>${escapeHtml(d.locationLabel||d.address||'')}</p></header><section class="qr-report-summary"><strong>${items.length} opportunity${items.length===1?'':'ies'} identified</strong><span>${high} high priority · ${withPhoto} photographed · existing signage can remain in place where suitable.</span></section><section class="qr-report-items">${items.map((item,i)=>{const url=welcomeSectionUrl(item.destination||'home');const qr=`https://api.qrserver.com/v1/create-qr-code/?size=220x220&margin=10&format=png&data=${encodeURIComponent(url)}`;return `<article class="qr-report-item"><div class="qr-report-number">${i+1}</div><div class="qr-report-photo-wrap">${item.photoKey?`<img class="qr-report-photo" data-report-photo-key="${escapeHtml(item.photoKey)}" alt="${escapeHtml(item.location||'Audit location')}">`:'<div class="qr-report-photo-empty">No audit photo</div>'}</div><div class="qr-report-copy"><h2>${escapeHtml(item.location||'Property touchpoint')}</h2><p>${escapeHtml(item.existing||'Existing guest information')}</p><strong>Recommended destination: ${escapeHtml(QR_TOUCHPOINT_DESTINATIONS[item.destination]||'Welcome home')}</strong><span>Status: ${escapeHtml(qrTouchpointStatusLabel(item.status))}</span><span>Sticker: “${escapeHtml(item.label||'Scan for details')}”</span>${item.note?`<small>${escapeHtml(item.note)}</small>`:''}<a class="no-print" href="${escapeHtml(url)}" target="_blank" rel="noopener">Open guest demo</a></div><img class="qr-report-qr" src="${escapeHtml(qr)}" alt="QR demo"></article>`}).join('')}</section><footer>Prepared with Papa Golf Platform · Proposal / walkthrough record</footer>`;await Promise.all([...host.querySelectorAll('[data-report-photo-key]')].map(async img=>{const a=await getAuditAsset(img.dataset.reportPhotoKey);if(a?.dataUrl)img.src=a.dataUrl}))}
...
const PAPA_GOLF_WELCOME_SCHEMA_VERSION = 2;
...
const WELCOME_DEFAULT_UNIT = {
  ...
  activities:[],
  presentation:{
    villaGuide:{published:true,policy:'customizable',defaultOrder:10},
    stayDetails:{published:true,policy:'customizable',defaultOrder:20},
    whatsOn:{published:true,policy:'customizable',defaultOrder:30},
    nearby:{published:true,policy:'customizable',defaultOrder:40},
    food:{published:true,policy:'customizable',defaultOrder:50},
    wellness:{published:true,policy:'customizable',defaultOrder:60},
    tours:{published:true,policy:'customizable',defaultOrder:70},
    transport:{published:true,policy:'customizable',defaultOrder:80},
    other:{published:true,policy:'customizable',defaultOrder:90},
    help:{published:true,policy:'system',defaultOrder:100}
  },
  overrideHost:false,
  host:'',
...
function getWelcomeUnit(){
  ...
  return {...WELCOME_DEFAULT_UNIT,...saved,id:papaGolfStableId('unit',saved.id),schemaVersion:PAPA_GOLF_WELCOME_SCHEMA_VERSION,stayDetails:{...WELCOME_DEFAULT_UNIT.stayDetails,...(saved.stayDetails||{})},presentation:{...WELCOME_DEFAULT_UNIT.presentation,...(saved.presentation||{})}};
}
function ensureWelcomeModelIdentity(){
  ...
  const u={...WELCOME_DEFAULT_UNIT,...rawU,id:papaGolfStableId('unit',rawU.id),schemaVersion:PAPA_GOLF_WELCOME_SCHEMA_VERSION,stayDetails:{...WELCOME_DEFAULT_UNIT.stayDetails,...(rawU.stayDetails||{})},presentation:{...WELCOME_DEFAULT_UNIT.presentation,...(rawU.presentation||{})}};
...
function buildCanonicalWelcomeModel(){
  ...
  return {
    ...
    presentation:{...WELCOME_DEFAULT_UNIT.presentation,...(u.presentation||{})},
    content,services,
...
function applyPapaGolfRoute(route){
  ...
  if(route==='map-view'){welcomeShow(document.getElementById('mapView'));document.getElementById('mapTabBtn')?.classList.add('active');return}
  if(route==='areas-view'){welcomeShow(document.getElementById('areasView'));document.getElementById('areasTabBtn')?.classList.add('active');return}
...
function welcomeSectionSetupState(id){
  ...
  if(id==='schedule'){
    ...
    if(count&&u.presentation?.whatsOn?.published===false)return {state:'hidden',label:'Hidden'};
...
function renderWelcomeSectionStatuses(){
  ...
  root.querySelectorAll('.pg-nav-section[data-pg-section]').forEach(section=>{
    ...
    badge.innerHTML=`<span class="pg-section-status-icon" aria-hidden="true">${state.state==='ready'?'✓':state.state==='more'?'✎':state.state==='hidden'?'◌':'·'}</span><span>${escapeHtml(state.label)}</span>`;
    badge.setAttribute('aria-label',state.state==='ready'?`Complete: ${state.label}`:state.state==='more'?`More information can be added: ${state.label}`:state.state==='hidden'?`Saved but hidden from guests`: `${state.label} section`);
  });
}
...
function clearWelcomeCategory(key){
  const u=getWelcomeUnit();
  const labels={villaGuide:'Villa Guide',stayDetails:'Stay Details',food:'Food & Drink',transport:'Transport',wellness:'Wellness',tours:'Tours & Experiences',other:'Other Guest Services',schedule:"What's On schedule"};
  if(!confirm(`Clear ${labels[key]||'these details'}? This removes the saved content from this device.`))return;
  const next={...u};
  if(key==='villaGuide')next.villaInfo='';
  if(key==='stayDetails')next.stayDetails={...WELCOME_DEFAULT_UNIT.stayDetails};
  if(key==='food')next.foodInfo='';
  if(key==='transport')next.transportInfo='';
  if(key==='wellness')next.wellnessInfo='';
  if(key==='tours')next.toursInfo='';
  if(key==='other')next.otherServices='';
  if(key==='schedule')next.activities=[];
  localStorage.setItem(WELCOME_UNIT_KEY,JSON.stringify(next));
  loadWelcomeEditor();renderGuestWelcome();
}
function loadWelcomeEditor(){
  ...
  const pr={...WELCOME_DEFAULT_UNIT.presentation,...(u.presentation||{})};
  [['showWelcomeVillaGuide','villaGuide'],['showWelcomeStayDetails','stayDetails'],['showWelcomeFood','food'],['showWelcomeTransport','transport'],['showWelcomeWellness','wellness'],['showWelcomeTours','tours'],['showWelcomeOther','other'],['showWelcomeWhatsOn','whatsOn'],['showWelcomeNearby','nearby']].forEach(([id,key])=>{const el=document.getElementById(id);if(el)el.checked=pr[key]?.published!==false});
...
function saveWelcomeUnit(){
  ...
  localStorage.setItem(WELCOME_UNIT_KEY,JSON.stringify({
    ...
    presentation:{...WELCOME_DEFAULT_UNIT.presentation,...(old.presentation||{}),villaGuide:{...WELCOME_DEFAULT_UNIT.presentation.villaGuide,...(old.presentation?.villaGuide||{}),published:document.getElementById('showWelcomeVillaGuide')?.checked!==false},stayDetails:{...WELCOME_DEFAULT_UNIT.presentation.stayDetails,...(old.presentation?.stayDetails||{}),published:document.getElementById('showWelcomeStayDetails')?.checked!==false},food:{...WELCOME_DEFAULT_UNIT.presentation.food,...(old.presentation?.food||{}),published:document.getElementById('showWelcomeFood')?.checked!==false},transport:{...WELCOME_DEFAULT_UNIT.presentation.transport,...(old.presentation?.transport||{}),published:document.getElementById('showWelcomeTransport')?.checked!==false},wellness:{...WELCOME_DEFAULT_UNIT.presentation.wellness,...(old.presentation?.wellness||{}),published:document.getElementById('showWelcomeWellness')?.checked!==false},tours:{...WELCOME_DEFAULT_UNIT.presentation.tours,...(old.presentation?.tours||{}),published:document.getElementById('showWelcomeTours')?.checked!==false},other:{...WELCOME_DEFAULT_UNIT.presentation.other,...(old.presentation?.other||{}),published:document.getElementById('showWelcomeOther')?.checked!==false},whatsOn:{...WELCOME_DEFAULT_UNIT.presentation.whatsOn,...(old.presentation?.whatsOn||{}),published:document.getElementById('showWelcomeWhatsOn')?.checked!==false},nearby:{...WELCOME_DEFAULT_UNIT.presentation.nearby,...(old.presentation?.nearby||{}),published:document.getElementById('showWelcomeNearby')?.checked!==false},help:{...WELCOME_DEFAULT_UNIT.presentation.help,...(old.presentation?.help||{}),published:true,policy:'system'}},
...
function setupWelcomeMenuReorder(root=document){
  ...
  root.querySelectorAll('[data-reorder-list="welcome"]').forEach(list=>{
    if(list.dataset.reorderReady==='1')return;
    list.dataset.reorderReady='1';
    let drag=null;
    const animateReflow=before=>{
      [...list.querySelectorAll('[data-menu-key]')].forEach(row=>{
        const prev=before.get(row);if(!prev)return;
        const next=row.getBoundingClientRect(),dy=prev.top-next.top;
        if(Math.abs(dy)<1)return;
        row.animate([{transform:`translateY(${dy}px)`},{transform:'translateY(0)'}],{duration:180,easing:'cubic-bezier(.2,.8,.2,1)'});
      });
    };
    const movePlaceholder=(hit,clientY)=>{
      if(!drag||!hit||hit===drag.active||hit.parentElement!==list||hit.classList.contains('guest-system-tile'))return;
      const before=new Map([...list.querySelectorAll('[data-menu-key]')].map(row=>[row,row.getBoundingClientRect()]));
      const r=hit.getBoundingClientRect();
      list.insertBefore(drag.placeholder,clientY<r.top+r.height/2?hit:hit.nextSibling);
      animateReflow(before);
    };
    const autoScroll=clientY=>{
      const edge=84,max=11;
      if(clientY<edge)window.scrollBy(0,-Math.min(max,(edge-clientY)/5));
      else if(clientY>innerHeight-edge)window.scrollBy(0,Math.min(max,(clientY-(innerHeight-edge))/5));
    };
    list.addEventListener('pointerdown',e=>{
      const h=e.target.closest('.guest-menu-drag');if(!h)return;
      const active=h.closest('[data-menu-key]');if(!active)return;
      e.preventDefault();e.stopPropagation();
      const r=active.getBoundingClientRect();
      const placeholder=document.createElement('div');
      placeholder.className='guest-menu-placeholder';
      placeholder.style.height=`${r.height}px`;
      active.after(placeholder);
      drag={active,handle:h,placeholder,pointerId:e.pointerId,grabOffset:e.clientY-r.top,left:r.left,width:r.width};
      active.classList.add('is-reordering');
      active.style.position='fixed';active.style.left=`${r.left}px`;active.style.top=`${r.top}px`;active.style.width=`${r.width}px`;active.style.height=`${r.height}px`;active.style.margin='0';
      document.body.classList.add('welcome-menu-dragging');
      try{h.setPointerCapture(e.pointerId)}catch{}
    });
    list.addEventListener('pointermove',e=>{
      if(!drag)return;
      e.preventDefault();e.stopPropagation();
      drag.active.style.top=`${e.clientY-drag.grabOffset}px`;
      autoScroll(e.clientY);
      const hit=document.elementsFromPoint(e.clientX,e.clientY).map(el=>el.closest?.('[data-menu-key]')).find(row=>row&&row!==drag.active&&row.parentElement===list);
      movePlaceholder(hit,e.clientY);
    });
    const finish=e=>{
      if(!drag)return;
      e?.preventDefault();e?.stopPropagation();
      const {active,handle,placeholder,pointerId}=drag;
      const target=placeholder.getBoundingClientRect();
      active.style.transition='top 150ms cubic-bezier(.2,.8,.2,1), left 150ms cubic-bezier(.2,.8,.2,1), transform 150ms ease, box-shadow 150ms ease';
      active.style.top=`${target.top}px`;active.style.left=`${target.left}px`;
      setTimeout(()=>{
        placeholder.replaceWith(active);
        active.classList.remove('is-reordering');
        active.removeAttribute('style');
        document.body.classList.remove('welcome-menu-dragging');
        saveWelcomeMenuOrder(list);
    ...
    list.addEventListener('keydown',e=>{const h=e.target.closest('.guest-menu-drag');if(!h||!['ArrowUp','ArrowDown'].includes(e.key))return;e.preventDefault();e.stopPropagation();const row=h.closest('[data-menu-key]');const peers=[...list.querySelectorAll('[data-menu-key]')];const i=peers.indexOf(row),j=e.key==='ArrowUp'?i-1:i+1;if(j<0||j>=peers.length)return;const before=new Map(peers.map(x=>[x,x.getBoundingClientRect()]));if(e.key==='ArrowUp')list.insertBefore(row,peers[j]);else list.insertBefore(row,peers[j].nextSibling);animateReflow(before);saveWelcomeMenuOrder(list);h.focus()});
  });
}
...
function renderGuestActivities(){
  const current=effectiveWelcome();const published=current.presentation?.whatsOn?.published!==false;
  const items=published?(Array.isArray(current.activities)?current.activities:[]).filter(x=>x.enabled!==false&&x.title):[];
...
function renderGuestWelcome(){
  ...
  if(stayTile)stayTile.classList.toggle('hidden',!hasStay||vis.stayDetails?.published===false);
  ...
  if(document.getElementById('guestVillaTile'))document.getElementById('guestVillaTile').classList.toggle('hidden',vis.villaGuide?.published===false||!d.villaInfo);
  if(document.getElementById('guestExploreTile'))document.getElementById('guestExploreTile').classList.toggle('hidden',vis.nearby?.published===false);
  if(foodTile)foodTile.classList.toggle('hidden',!hasFood||vis.food?.published===false);
  if(wellnessTile)wellnessTile.classList.toggle('hidden',!hasWellness||vis.wellness?.published===false);
  if(toursTile)toursTile.classList.toggle('hidden',!hasTours||vis.tours?.published===false);
  if(transportTile)transportTile.classList.toggle('hidden',!hasTransport||vis.transport?.published===false);
  ...
  if(em)em.innerHTML=d.emergency?`<p><strong>Emergency information</strong><br>${escapeHtml(d.emergency).replace(/\n/g,'<br>')}</p>`:'<p class="muted">Emergency information has not been added yet.</p>';
  const emHost=document.getElementById('guestEmergencyHost');if(emHost)emHost.innerHTML=d.host?`<p><strong>Host / property contact</strong><br>${escapeHtml(d.host)}</p>`:'<p class="muted">Host contact has not been added yet.</p>';
...
const PAPA_GOLF_AFFILIATE_ASSET_DB='papa-golf-assets-v01';
const PAPA_GOLF_AFFILIATE_EVENT_KEY='papaGolfAffiliateEventsV1';
const PAPA_GOLF_AFFILIATE_LOGO_KEY='affiliate-primary-logo';
let affiliateEventView='upcoming';
let editingAffiliateEventId='';
let affiliatePromotionSelection=[];
function openAffiliateAssetDb(){return new Promise((resolve,reject)=>{const req=indexedDB.open(PAPA_GOLF_AFFILIATE_ASSET_DB,1);req.onupgradeneeded=()=>{if(!req.result.objectStoreNames.contains('brandAssets'))req.result.createObjectStore('brandAssets',{keyPath:'key'})};req.onsuccess=()=>resolve(req.result);req.onerror=()=>reject(req.error)})}
async function saveAffiliateAsset(key,dataUrl,meta={}){const db=await openAffiliateAssetDb();return new Promise((resolve,reject)=>{const tx=db.transaction('brandAssets','readwrite');tx.objectStore('brandAssets').put({key,dataUrl,...meta,updatedAt:new Date().toISOString()});tx.oncomplete=()=>{db.close();resolve(true)};tx.onerror=()=>{db.close();reject(tx.error)}})}
async function getAffiliateAsset(key){try{const db=await openAffiliateAssetDb();return await new Promise((resolve,reject)=>{const tx=db.transaction('brandAssets','readonly'),req=tx.objectStore('brandAssets').get(key);req.onsuccess=()=>{db.close();resolve(req.result||null)};req.onerror=()=>{db.close();reject(req.error)}})}catch{return null}}
function getAffiliateEvents(){const x=readWelcomeJson(PAPA_GOLF_AFFILIATE_EVENT_KEY,[]);return Array.isArray(x)?x:[]}
function saveAffiliateEvents(items){localStorage.setItem(PAPA_GOLF_AFFILIATE_EVENT_KEY,JSON.stringify(items))}
function affiliateEventDateValue(e){const raw=[e.date,e.time||'23:59'].filter(Boolean).join('T');const d=new Date(raw);return Number.isNaN(d.getTime())?Infinity:d.getTime()}
function isAffiliateEventPast(e){if(!e.date)return false;const end=new Date(`${e.date}T${e.endTime||e.time||'23:59'}`);return !Number.isNaN(end.getTime())&&end.getTime()<Date.now()}
function formatAffiliateEventWhen(e){if(!e.date)return 'Date to be confirmed';const d=new Date(`${e.date}T${e.time||'12:00'}`);const date=Number.isNaN(d.getTime())?e.date:d.toLocaleDateString(undefined,{weekday:'short',day:'numeric',month:'short'});return `${date}${e.time?' · '+e.time:''}`}
function renderAffiliateEventLibrary(){const host=document.getElementById('affiliateEventLibrary');if(!host)return;const filter=document.getElementById('affiliateEventFilter')?.value||'all';let items=getAffiliateEvents().filter(e=>(affiliateEventView==='past')===isAffiliateEventPast(e));if(filter!=='all')items=items.filter(e=>e.type===filter);items.sort((a,b)=>affiliateEventDateValue(a)-affiliateEventDateValue(b));host.innerHTML=items.length?items.map(e=>`<div class="affiliate-event-card ${isAffiliateEventPast(e)?'affiliate-event-past':''}" data-event-id="${escapeHtml(e.id)}">${affiliateEventView==='upcoming'?`<input class="affiliate-event-select" type="checkbox" aria-label="Select ${escapeHtml(e.title)}">`:''}<div class="affiliate-event-main"><strong>${escapeHtml(e.title||'Untitled event')}</strong><div class="affiliate-event-meta">${escapeHtml(formatAffiliateEventWhen(e))} · ${escapeHtml((e.type||'event').replace('-',' '))}</div>${e.offer?`<div class="affiliate-event-offer">${escapeHtml(e.offer)}</div>`:''}</div><div class="affiliate-event-actions"><button class="secondary-btn" type="button" data-edit-affiliate-event="${escapeHtml(e.id)}">Edit</button>${affiliateEventView==='past'?'<span class="small muted">Archived</span>':''}</div></div>`).join(''):`<div class="affiliate-event-empty muted">${affiliateEventView==='past'?'No past events yet.':'No upcoming events yet. Add the first thing your venue is doing.'}</div>`}
function clearAffiliateEventEditor(){editingAffiliateEventId='';['affiliatePromoTitle','affiliatePromoDate','affiliatePromoTime','affiliatePromoOffer'].forEach(id=>{const e=document.getElementById(id);if(e)e.value=''});['affiliatePromoSound','affiliatePromoBigScreen','affiliatePromoMultipleScreens'].forEach(id=>{const e=document.getElementById(id);if(e)e.checked=false});document.querySelectorAll('#affiliatePromoTypes button').forEach(x=>x.classList.remove('active'));updateAffiliateQuickPick('');affiliateStatus('Ready for another event.')}
function editAffiliateEvent(id){const e=getAffiliateEvents().find(x=>x.id===id);if(!e)return;editingAffiliateEventId=id;const set=(id,v)=>{const n=document.getElementById(id);if(n)n.value=v||''};set('affiliatePromoTitle',e.title);set('affiliatePromoDate',e.date);set('affiliatePromoTime',e.time);set('affiliatePromoOffer',e.offer);document.querySelectorAll('#affiliatePromoTypes button').forEach(x=>x.classList.toggle('active',x.dataset.promoType===e.type));updateAffiliateQuickPick(e.type);document.getElementById('affiliatePromoSound').checked=(e.options||[]).includes('Sound on');document.getElementById('affiliatePromoBigScreen').checked=(e.options||[]).includes('Big screen');document.getElementById('affiliatePromoMultipleScreens').checked=(e.options||[]).includes('Multiple screens');affiliateStatus('Editing saved event. Save Event when finished.');document.getElementById('affiliatePromoTitle')?.scrollIntoView({behavior:'smooth',block:'center'})}
function saveCurrentAffiliateEvent(){const p=currentAffiliatePromotion();if(!p.title||p.title==='Your promotion'){affiliateStatus('Add an event title before saving.');return}const items=getAffiliateEvents(),now=new Date().toISOString();const record={...p,id:editingAffiliateEventId||`evt-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,updatedAt:now};const i=items.findIndex(x=>x.id===record.id);if(i>=0)items[i]={...items[i],...record};else items.push({...record,createdAt:now});saveAffiliateEvents(items);editingAffiliateEventId='';renderAffiliateEventLibrary();affiliateStatus('Event saved. It is now available for future promotions.');}
async function createPromotionFromSelected(){const ids=[...document.querySelectorAll('.affiliate-event-card .affiliate-event-select:checked')].map(x=>x.closest('.affiliate-event-card')?.dataset.eventId).filter(Boolean);if(!ids.length){affiliateStatus('Tick one or more upcoming events first.');return}affiliatePromotionSelection=ids;const items=getAffiliateEvents().filter(e=>ids.includes(e.id)).sort((a,b)=>affiliateEventDateValue(a)-affiliateEventDateValue(b));const host=document.getElementById('affiliatePromoPreview');host.classList.remove('hidden');host.innerHTML=`<div class="promo-preview-kicker">PAPA GOLF · ${items.length===1?'SINGLE EVENT':'MULTI-EVENT PROGRAMME'}</div><h4>${items.length===1?escapeHtml(items[0].title):escapeHtml(`${items.length} events selected`)}</h4>${items.map(e=>`<div style="margin:.7rem 0"><strong>${escapeHtml(e.title)}</strong><div>${escapeHtml(formatAffiliateEventWhen(e))}</div>${e.offer?`<div>${escapeHtml(e.offer)}</div>`:''}</div>`).join('')}<small>${escapeHtml(getPapaGolfAffiliateProfile().businessName||'Affiliate business')} · Powered by Papa Golf</small>`;await renderAffiliateCreative();affiliateStatus(`${items.length} event${items.length===1?'':'s'} selected. The Brand Studio artwork below now uses this exact selection.`);document.getElementById('affiliateCreativeCanvas')?.scrollIntoView({behavior:'smooth',block:'center'})}
const PAPA_GOLF_AFFILIATE_PROFILE_KEY='papaGolfAffiliateProfileV1';
let pendingAffiliateLogo=null;
...
function affiliateStatus(message){const st=document.getElementById('affiliateProfileStatus');if(st)st.textContent=message||''}
function renderAffiliateLogoPreview(dataUrl,label='Saved Brand Kit logo'){
  const box=document.getElementById('affiliateLogoPreview');if(!box)return;
  box.innerHTML=dataUrl?`<img src="${dataUrl}" alt="Affiliate logo preview"><div class="affiliate-logo-state">${escapeHtml(label)}</div>`:'<span>No logo saved yet</span>';
}
async function loadAffiliateProfile(){
  ...
  pendingAffiliateLogo=null;let logo=a.logoDataUrl||'';const asset=await getAffiliateAsset(a.logoAssetKey||PAPA_GOLF_AFFILIATE_LOGO_KEY);if(asset?.dataUrl)logo=asset.dataUrl;renderAffiliateLogoPreview(logo,logo?'Saved Brand Kit logo':'');
}
function compressAffiliateLogo(file){
  return new Promise((resolve,reject)=>{
    if(!file||!file.type?.startsWith('image/'))return reject(new Error('Choose an image file.'));
    const reader=new FileReader();reader.onerror=()=>reject(reader.error||new Error('Could not read image.'));
    reader.onload=()=>{const img=new Image();img.onerror=()=>reject(new Error('Could not decode image.'));img.onload=()=>{
      const max=900,scale=Math.min(1,max/Math.max(img.naturalWidth||1,img.naturalHeight||1));
      const w=Math.max(1,Math.round(img.naturalWidth*scale)),h=Math.max(1,Math.round(img.naturalHeight*scale));
      const c=document.createElement('canvas');c.width=w;c.height=h;const ctx=c.getContext('2d');ctx.drawImage(img,0,0,w,h);
      let dataUrl='';try{dataUrl=c.toDataURL('image/webp',0.86)}catch{}if(!dataUrl||dataUrl==='data:,')dataUrl=c.toDataURL('image/jpeg',0.86);
      resolve({dataUrl,name:file.name,width:w,height:h,originalBytes:file.size||0});
    };img.src=String(reader.result||'')};reader.readAsDataURL(file);
  });
}
async function selectAffiliateLogo(file){
  if(!file)return;affiliateStatus('Preparing logo preview…');
  try{pendingAffiliateLogo=await compressAffiliateLogo(file);renderAffiliateLogoPreview(pendingAffiliateLogo.dataUrl,'Selected · tap Save Affiliate Brand Kit');affiliateStatus('Logo selected. Save the Brand Kit to keep it.');}
  catch(error){pendingAffiliateLogo=null;affiliateStatus(`Logo could not be prepared: ${error.message||error}`)}
}
async function saveAffiliateProfile(){
  ...
  if(pendingAffiliateLogo){await saveAffiliateAsset(PAPA_GOLF_AFFILIATE_LOGO_KEY,pendingAffiliateLogo.dataUrl,{name:pendingAffiliateLogo.name,width:pendingAffiliateLogo.width,height:pendingAffiliateLogo.height});a.logoAssetKey=PAPA_GOLF_AFFILIATE_LOGO_KEY;delete a.logoDataUrl;a.logoName=pendingAffiliateLogo.name;a.logoWidth=pendingAffiliateLogo.width;a.logoHeight=pendingAffiliateLogo.height;}
  try{localStorage.setItem(PAPA_GOLF_AFFILIATE_PROFILE_KEY,JSON.stringify(a));pendingAffiliateLogo=null;await loadAffiliateProfile();affiliateStatus('Affiliate Brand Kit saved on this device. Logo stored in the dedicated Brand Asset library.');}
  catch(error){affiliateStatus(`Brand Kit was not saved: ${error?.name==='QuotaExceededError'?'logo is still too large for local storage':(error.message||error)}`)}
}
function loadAffiliateDemo(){const existing=getPapaGolfAffiliateProfile();const a={...existing,businessName:'Papa Golf Test Bar',businessType:'bar',brandPrimary:'#111111',brandAccent:'#D4AF37',brandStyle:'Relaxed tropical bar · clean black and gold · bold event photography',contactLine:'Bangrak, Samui · Papa Golf Founding Affiliate',updatedAt:new Date().toISOString()};localStorage.setItem(PAPA_GOLF_AFFILIATE_PROFILE_KEY,JSON.stringify(a));loadAffiliateProfile();affiliateStatus('Test Bar example loaded. Existing saved logo kept. Edit anything and save.')}
function currentAffiliatePromotion(){const type=document.querySelector('#affiliatePromoTypes button.active')?.dataset.promoType||'promotion';return{type,title:affiliateVal('affiliatePromoTitle')||'Your promotion',date:affiliateVal('affiliatePromoDate'),time:affiliateVal('affiliatePromoTime'),offer:affiliateVal('affiliatePromoOffer'),options:[['Sound on','affiliatePromoSound'],['Big screen','affiliatePromoBigScreen'],['Multiple screens','affiliatePromoMultipleScreens']].filter(([,id])=>document.getElementById(id)?.checked).map(([x])=>x)}}
function renderAffiliatePromoPreview(){const a=getPapaGolfAffiliateProfile();const p=currentAffiliatePromotion();const host=document.getElementById('affiliatePromoPreview');if(!host)return;host.classList.remove('hidden');host.innerHTML=`<div class="promo-preview-kicker">${escapeHtml(p.type.replace('-',' '))} · PAPA GOLF STRUCTURED PROMOTION</div><h4>${escapeHtml(p.title)}</h4><div>${escapeHtml([p.date,p.time].filter(Boolean).join(' · ')||'Date / time not set')}</div>${p.offer?`<strong>${escapeHtml(p.offer)}</strong>`:''}${p.options.length?`<div class="promo-preview-options">${p.options.map(x=>`<span>✓ ${escapeHtml(x)}</span>`).join('')}</div>`:''}<small>${escapeHtml(a.businessName||'Affiliate business')} · Powered by Papa Golf</small><p>This same structured record now also drives the first Brand Studio export below.</p>`;renderAffiliateCreative()}
function hexOr(value,fallback){const v=String(value||'').trim();return /^#[0-9a-f]{6}$/i.test(v)?v:fallback}
function wrapCanvasText(ctx,text,maxWidth){const words=String(text||'').split(/\s+/);const lines=[];let line='';for(const word of words){const test=line?line+' '+word:word;if(ctx.measureText(test).width>maxWidth&&line){lines.push(line);line=word}else line=test}if(line)lines.push(line);return lines}
async function drawAffiliateLogo(ctx,dataUrl,x,y,maxW,maxH){if(!dataUrl)return false;return new Promise(resolve=>{const img=new Image();img.onload=()=>{const scale=Math.min(maxW/img.naturalWidth,maxH/img.naturalHeight,1);const w=img.naturalWidth*scale,h=img.naturalHeight*scale;ctx.drawImage(img,x,y,w,h);resolve(true)};img.onerror=()=>resolve(false);img.src=dataUrl})}
async function renderAffiliateCreative(){
  const canvas=document.getElementById('affiliateCreativeCanvas');if(!canvas)return;
  const a=getPapaGolfAffiliateProfile(),current=currentAffiliatePromotion();
  const selected=affiliatePromotionSelection.length?getAffiliateEvents().filter(e=>affiliatePromotionSelection.includes(e.id)).sort((x,y)=>affiliateEventDateValue(x)-affiliateEventDateValue(y)):[];
  const items=selected.length?selected:[current];
  const logoAsset=await getAffiliateAsset(a.logoAssetKey||PAPA_GOLF_AFFILIATE_LOGO_KEY);const affiliateLogo=logoAsset?.dataUrl||a.logoDataUrl||'';
  const format=document.getElementById('affiliateCreativeFormat')?.value||'square';const dims=format==='story'?[1080,1920]:[1080,1080];canvas.width=dims[0];canvas.height=dims[1];
  const [w,h]=dims,ctx=canvas.getContext('2d');const primary=hexOr(a.brandPrimary,'#111111'),accent=hexOr(a.brandAccent,'#D4AF37');
  ctx.fillStyle=primary;ctx.fillRect(0,0,w,h);ctx.fillStyle=accent;ctx.fillRect(0,0,w,18);ctx.fillRect(0,h-18,w,18);
  await drawAffiliateLogo(ctx,affiliateLogo,64,56,330,165);
  let y=format==='story'?285:250;
  ctx.fillStyle=accent;ctx.font='700 29px system-ui, sans-serif';ctx.fillText(items.length>1?'WHAT’S ON · '+items.length+' EVENTS':(items[0].type||'PROMOTION').replace('-',' ').toUpperCase(),64,y);y+=62;
  const maxY=h-175;
  for(let i=0;i<items.length && y<maxY;i++){
    const p=items[i];
    ctx.fillStyle='#ffffff';ctx.font=items.length>1?'800 48px system-ui, sans-serif':'800 68px system-ui, sans-serif';
    for(const line of wrapCanvasText(ctx,p.title||'Event',w-128).slice(0,2)){ctx.fillText(line,64,y);y+=items.length>1?56:80}
    ctx.fillStyle='#d8d8d8';ctx.font=items.length>1?'500 30px system-ui, sans-serif':'500 36px system-ui, sans-serif';ctx.fillText(formatAffiliateEventWhen(p),64,y);y+=43;
    if(p.offer){ctx.fillStyle=accent;ctx.font=items.length>1?'750 32px system-ui, sans-serif':'800 48px system-ui, sans-serif';for(const line of wrapCanvasText(ctx,p.offer,w-128).slice(0,2)){ctx.fillText(line,64,y);y+=items.length>1?39:57}}
    if(items.length===1 && p.options?.length){y+=8;ctx.fillStyle='#ffffff';ctx.font='600 30px system-ui, sans-serif';ctx.fillText(p.options.map(x=>'✓ '+x).join('   '),64,y);y+=44}
    if(i<items.length-1){ctx.fillStyle='rgba(255,255,255,.18)';ctx.fillRect(64,y+10,w-128,2);y+=48}
  }
  if(y>=maxY && items.length>1){ctx.fillStyle=accent;ctx.font='650 25px system-ui, sans-serif';ctx.fillText('More details on the Papa Golf venue page',64,maxY-12)}
  const footerY=h-92;ctx.fillStyle='#bdbdbd';ctx.font='500 27px system-ui, sans-serif';ctx.fillText(a.businessName||'Affiliate business',64,footerY);ctx.textAlign='right';ctx.fillStyle=accent;ctx.font='700 27px system-ui, sans-serif';ctx.fillText('POWERED BY PAPA GOLF',w-64,footerY);ctx.textAlign='left';
  const note=document.getElementById('affiliateCreativeStatus');if(note)note.textContent=items.length>1?`${items.length} selected events are included in this artwork. Change the ticked events and tap Create Promotion from Selected again to rebuild it.`:'Preview generated locally. Campaign QR will be added when this promotion is published to a public Papa Golf URL.';
}
function downloadAffiliateCreative(){const canvas=document.getElementById('affiliateCreativeCanvas');if(!canvas)return;const a=document.createElement('a');const name=(affiliateVal('affiliateBusinessName')||'papa-golf-affiliate').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'');a.download=`${name||'papa-golf'}-promotion.png`;a.href=canvas.toDataURL('image/png');document.body.appendChild(a);a.click();a.remove()}

const PAPA_GOLF_EVENT_CATALOG={
  'f1-monaco':{title:'F1 · Monaco Grand Prix'},'f1-singapore':{title:'F1 · Singapore Grand Prix'},'epl-feature':{title:'Premier League · Featured match'},'afl-feature':{title:'AFL · Featured match'},'nrl-feature':{title:'NRL · Featured match'}
};
function updateAffiliateQuickPick(type){const box=document.getElementById('affiliateSportsQuickPick');if(box)box.classList.toggle('hidden',type!=='sport')}
function applyAffiliateSportEvent(){const item=PAPA_GOLF_EVENT_CATALOG[document.getElementById('affiliateSportEventSelect')?.value];if(!item)return;const title=document.getElementById('affiliatePromoTitle');if(title)title.value=item.title;affiliateStatus('Papa Golf event selected. Add the venue-specific date/time, showing details and special.')}
function createPublicPromoLink(){const a=getPapaGolfAffiliateProfile(),p=currentAffiliatePromotion();const params=new URLSearchParams({type:p.type,title:p.title,date:p.date,time:p.time,offer:p.offer,options:p.options.join('|'),venue:a.businessName||'Papa Golf Affiliate',contact:a.contactLine||'',accent:hexOr(a.brandAccent,'#D4AF37')});const url=new URL('promotion.html',location.href);url.search=params.toString();const host=document.getElementById('affiliatePublicPromoLink');if(host){host.classList.remove('hidden');host.innerHTML=`<strong>Public test promotion</strong><p class="small muted">This link contains only the promotion details shown here — no private admin data or saved logo.</p><a class="secondary-btn" href="${escapeHtml(url.href)}" target="_blank" rel="noopener">Open Public Promotion</a><button id="copyPublicPromoLinkBtn" class="secondary-btn" type="button">Copy Link</button>`;document.getElementById('copyPublicPromoLinkBtn')?.addEventListener('click',async()=>{try{await navigator.clipboard.writeText(url.href);affiliateStatus('Public promotion link copied. Send it to another phone for the cross-device test.')}catch{affiliateStatus('Open the public promotion and copy its address from Safari.') }})}}
function initAffiliateStudio(){loadAffiliateProfile();renderAffiliateEventLibrary();document.getElementById('saveAffiliateProfileBtn')?.addEventListener('click',saveAffiliateProfile);document.getElementById('loadAffiliateDemoBtn')?.addEventListener('click',loadAffiliateDemo);document.getElementById('affiliateLogoInput')?.addEventListener('change',e=>selectAffiliateLogo(e.target.files?.[0]));document.getElementById('affiliatePromoTypes')?.addEventListener('click',e=>{const b=e.target.closest('[data-promo-type]');if(!b)return;document.querySelectorAll('#affiliatePromoTypes button').forEach(x=>x.classList.remove('active'));b.classList.add('active');updateAffiliateQuickPick(b.dataset.promoType)});document.getElementById('affiliateSportEventSelect')?.addEventListener('change',applyAffiliateSportEvent);document.getElementById('saveAffiliateEventBtn')?.addEventListener('click',saveCurrentAffiliateEvent);document.getElementById('newAffiliateEventBtn')?.addEventListener('click',clearAffiliateEventEditor);document.getElementById('createPromoFromSelectedBtn')?.addEventListener('click',createPromotionFromSelected);document.getElementById('showAffiliateUpcomingBtn')?.addEventListener('click',()=>{affiliateEventView='upcoming';document.getElementById('showAffiliateUpcomingBtn').classList.add('active');document.getElementById('showAffiliatePastBtn').classList.remove('active');renderAffiliateEventLibrary()});document.getElementById('showAffiliatePastBtn')?.addEventListener('click',()=>{affiliateEventView='past';document.getElementById('showAffiliatePastBtn').classList.add('active');document.getElementById('showAffiliateUpcomingBtn').classList.remove('active');renderAffiliateEventLibrary()});document.getElementById('affiliateEventFilter')?.addEventListener('change',renderAffiliateEventLibrary);document.getElementById('affiliateEventLibrary')?.addEventListener('click',e=>{const b=e.target.closest('[data-edit-affiliate-event]');if(b)editAffiliateEvent(b.dataset.editAffiliateEvent)});document.getElementById('previewAffiliatePromoBtn')?.addEventListener('click',()=>{affiliatePromotionSelection=[];renderAffiliatePromoPreview();renderAffiliateCreative()});document.getElementById('affiliateCreativeFormat')?.addEventListener('change',renderAffiliateCreative);document.getElementById('downloadAffiliateCreativeBtn')?.addEventListener('click',downloadAffiliateCreative);document.getElementById('createPublicPromoLinkBtn')?.addEventListener('click',createPublicPromoLink)}
...
function initWelcomeModule(){
  ...
  document.getElementById('welcomeModule')?.addEventListener('click',event=>{const btn=event.target.closest('[data-clear-welcome]');if(btn)clearWelcomeCategory(btn.dataset.clearWelcome)});
  ...
  document.getElementById('cancelQrTouchpointEditBtn')?.addEventListener('click',resetQrTouchpointForm);
  document.getElementById('qrTouchpointCameraInput')?.addEventListener('change',e=>selectQrTouchpointPhoto(e.target.files?.[0]));
  document.getElementById('qrTouchpointPhotoInput')?.addEventListener('change',e=>selectQrTouchpointPhoto(e.target.files?.[0]));
  ...
  document.getElementById('qrTouchpointList')?.addEventListener('click',event=>{
    ...
    const edit=event.target.closest('[data-edit-touchpoint]');
    if(edit){editQrTouchpoint(edit.dataset.editTouchpoint);return}
  ...
  ['photosTabBtn','mapTabBtn','areasTabBtn'].forEach(id=>{
    document.getElementById(id)?.addEventListener('click',()=>pushPapaGolfRoute(id==='photosTabBtn'?'photos-home':id==='mapTabBtn'?'map-view':'areas-view'));
  });
}
